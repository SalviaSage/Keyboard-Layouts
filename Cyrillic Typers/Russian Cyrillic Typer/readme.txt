...
The symbols on the top row number keys !@#$%^&*()= can still be typed by pressing the ScrollLock (Kana) + 1234567890 keys.
...
The dash key is typed by pressing AltGr + ( \ ) key located under the backspace key.
The currency symbol (₽) can be typed with AltGr+Shift+4($).
The currency symbol (₴) can be typed with AltGr+Shift+6(^).
...
We use this program as a base and run the Hotstrings.exe application.
From there, we load the "IME Cyrillic (Early Cyrillic).csv" library.
