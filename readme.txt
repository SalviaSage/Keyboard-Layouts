...
The keyboard can already be changed with winkey+space anyway, so it doesn't have to be set.
	...
	How to move the keyboard blinker (the caret) to the right of the screen?
The caret can be moved positions to the right side in most Windows programs.
To set a paragraph direction to:

Right to left: Control+Right Shift
Left to right: Control+Left Shift (default)
